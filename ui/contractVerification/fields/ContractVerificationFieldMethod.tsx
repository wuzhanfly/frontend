...
const ContractVerificationFieldMethod = ({ methods }: Props) => {
  ...
  const renderPopoverListItem = React.useCallback((method: SmartContractVerificationMethod) => {
    switch (method) {
      ...
      case 'sourcify':
        return <List.Item key={ method }>{ t('contract_verification.common.sourcify_description') } <Link href="https://sourcify.dev/" external noIcon className="dark">Sourcify</Link>.</List.Item>;
      case 'standard-input':
        return (
  ...
  return (
    <>
      <Heading level="2" mt={{ base: 10, lg: 6 }} gridColumn={{ lg: '1 / 3' }}>
        { t('contract_verification.common.currently_supports_methods', { count: methods.length }) }{ nbsp }contract verification methods
        <Hint
