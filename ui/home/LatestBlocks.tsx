    ...
    content = (
      <>
        ...
        <Flex justifyContent="center">
          <Link textStyle="sm" href={ route({ pathname: '/blocks' }) }>{ t('transactions.common.view_all_blocks') }</Link>
        </Flex>
      </>
    );
  }
    ...
    <Box width={{ base: '100%', lg: '280px' }} flexShrink={ 0 }>
      ...
      { statsQueryResult.data?.network_utilization_percentage !== undefined && (
        <Skeleton loading={ statsQueryResult.isPlaceholderData } mt={ 2 } display="inline-block" textStyle="sm">
          <Text as="span">
            { t('transactions.common.network_utilization', { nbsp }) }
          </Text>
      ...
      { statsQueryResult.data?.celo && (
        <Box whiteSpace="pre-wrap" textStyle="sm" mt={ 2 }>
          <span>{ t('transactions.common.current_epoch') } </span>
