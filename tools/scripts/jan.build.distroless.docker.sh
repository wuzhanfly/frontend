...
docker build -f Dockerfile_distroless_minimal \
  --progress=plain \
  --no-cache \
  --build-arg GIT_COMMIT_SHA=$GIT_COMMIT_SHA \
  --build-arg GIT_TAG=$GIT_TAG \
  -t blockscout-frontend:distroless-minimal-local ./ 2>&1 | tee -a $LOG_FILE
...
if [ $BUILD_RESULT -eq 0 ]; then
  ...
  docker save -o "$TAR_FILE" blockscout-frontend:distroless-minimal-local 2>&1 | tee -a $LOG_FILE
